...
import markdown2 # بۆ خاندنا فایلێن TOS و Privacy

# --- رێکخستنا Intents ---
intents = discord.Intents.all()

# --- فەنکشنا خاندنا فایلێن یاسایی (ب تەنێ بۆ لینکێن Railway) ---
def render_legal_page(title, file_path):
    ...
    <head>
        ...
        <style>body{{background:#1a1a1a;color:white;padding:40px;line-height:1.6;}}h1{{color:#5865F2;text-align:center;}}</style>
    </head>
    <body><div style="max-width:800px;margin:auto;"><h1>{title}</h1>{html_content}</div></body>
    </html>
    """
...
class MyBot(commands.Bot):
    ...
    async def setup_hook(self):
        # --- ١. بەشێ وێب سێرڤەری (بۆ چاککرنا لینکێن Railway) ---
        app = web.Application()
        app.router.add_get('/', lambda r: web.Response(text="<h1>HMB Bot is Online</h1>", content_type='text/html'))
        ...
        await runner.setup()
        port = int(os.getenv("PORT", 8080)) # Railway پێدڤی ب ڤێ پۆرتێ هەیە
        site = web.TCPSite(runner, '0.0.0.0', port)
        await site.start()
        print(f"✅ Web Server started on port {port}")

        # --- ٢. کارێن بەری نوکە یێن بۆتی (Sync + Cleanup) ---
